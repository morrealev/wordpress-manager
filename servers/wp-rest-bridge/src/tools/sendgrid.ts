...
export const sendgridTools: Tool[] = [
  {
    name: "sg_send_email",
...
export const sendgridHandlers: Record<string, Function> = {
  sg_send_email: async (params: z.infer<typeof sgSendEmailSchema>) => {
    if (!hasSendGrid()) {
  ...
  sg_list_templates: async (params: z.infer<typeof sgListTemplatesSchema>) => {
    if (!hasSendGrid()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "SendGrid not configured. Add sendgrid_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
      ...
      if (generations) query.generations = generations;
  ...
  sg_get_template: async (params: z.infer<typeof sgGetTemplateSchema>) => {
    if (!hasSendGrid()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "SendGrid not configured. Add sendgrid_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  sg_list_contacts: async (params: z.infer<typeof sgListContactsSchema>) => {
    if (!hasSendGrid()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "SendGrid not configured. Add sendgrid_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  sg_add_contacts: async (params: z.infer<typeof sgAddContactsSchema>) => {
    if (!hasSendGrid()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "SendGrid not configured. Add sendgrid_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  sg_get_stats: async (params: z.infer<typeof sgGetStatsSchema>) => {
    if (!hasSendGrid()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "SendGrid not configured. Add sendgrid_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
      const { start_date, end_date, aggregated_by } = params;
      const query: Record<string, any> = { start_date };
      if (end_date) query.end_date = end_date;
      if (aggregated_by) query.aggregated_by = aggregated_by;
      const qs = Object.entries(query).map(([k, v]) => `${k}=${v}`).join('&');
