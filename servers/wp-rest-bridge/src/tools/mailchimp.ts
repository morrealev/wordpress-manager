...
export const mailchimpTools: Tool[] = [
  {
...
export const mailchimpHandlers: Record<string, Function> = {
  mc_list_audiences: async (_params: z.infer<typeof mcListAudiencesSchema>) => {
    if (!hasMailchimp()) {
  ...
  mc_get_audience_members: async (params: z.infer<typeof mcGetAudienceMembersSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  mc_create_campaign: async (params: z.infer<typeof mcCreateCampaignSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  mc_update_campaign_content: async (params: z.infer<typeof mcUpdateCampaignContentSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  mc_send_campaign: async (params: z.infer<typeof mcSendCampaignSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  mc_get_campaign_report: async (params: z.infer<typeof mcGetCampaignReportSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  mc_add_subscriber: async (params: z.infer<typeof mcAddSubscriberSchema>) => {
    if (!hasMailchimp()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Mailchimp not configured. Add mailchimp_api_key to WP_SITES_CONFIG." }] } };
    }
    try {
      const { list_id, email_address, status, merge_fields, tags } = params;
      const body: Record<string, any> = { email_address, status };
