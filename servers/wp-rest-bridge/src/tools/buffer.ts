...
export const bufferHandlers: Record<string, Function> = {
  buf_list_profiles: async (_params: z.infer<typeof bufListProfilesSchema>) => {
    if (!hasBuffer()) {
  ...
  buf_create_update: async (params: z.infer<typeof bufCreateUpdateSchema>) => {
    if (!hasBuffer()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Buffer not configured. Add buffer_access_token to WP_SITES_CONFIG." }] } };
    }
    try {
      const { profile_ids, text, media, scheduled_at, shorten } = params;
      const body: Record<string, any> = { profile_ids, text };
  ...
  buf_list_pending: async (params: z.infer<typeof bufListPendingSchema>) => {
    if (!hasBuffer()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Buffer not configured. Add buffer_access_token to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  buf_list_sent: async (params: z.infer<typeof bufListSentSchema>) => {
    if (!hasBuffer()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Buffer not configured. Add buffer_access_token to WP_SITES_CONFIG." }] } };
    }
    try {
  ...
  buf_get_analytics: async (params: z.infer<typeof bufGetAnalyticsSchema>) => {
    if (!hasBuffer()) {
      return { toolResult: { isError: true, content: [{ type: "text", text: "Buffer not configured. Add buffer_access_token to WP_SITES_CONFIG." }] } };
    }
    try {
